...
std::string createjson(std::string AppName, std::string OutputFoldername,
                       int NumIterations, int NumDSERandomSamples,
                       std::vector<HMInputParamBase *> &InParams,
                       std::vector<std::string> Objectives,
                       std::string optimization, std::string count);
...
int collectInputParams(std::vector<HMInputParamBase *> &InParams,
                       std::string test_name);
void deleteInputParams(std::vector<HMInputParamBase *> &InParams);
auto findHMParamByKey(std::vector<HMInputParamBase *> &InParams,
                      std::string Key);
void setInputValue(HMInputParamBase *Param, std::string ParamVal);
taco::IndexStmt scheduleTTMCPU(taco::IndexStmt stmt, taco::Tensor<double> B,
                               int CHUNK_SIZE, int UNROLL_FACTOR, int order);
HMObjective
calculateObjectiveSpMVDense(std::vector<HMInputParamBase *> &InputParams,
                            std::string matrix_name, std::ofstream &logger);
HMObjective
calculateObjectiveSpMVSparse(std::vector<HMInputParamBase *> &InputParams,
                             std::string matrix_name, std::ofstream &logger);
HMObjective
calculateObjectiveSpMMDense(std::vector<HMInputParamBase *> &InputParams,
                            std::string matrix_name, std::ofstream &logger);
HMObjective
calculateObjectiveSDDMMDense(std::vector<HMInputParamBase *> &InputParams,
                             std::string matrix_name, std::ofstream &logger);
HMObjective
calculateObjectiveTTVDense(std::vector<HMInputParamBase *> &InputParams,
                           std::string matrix_name, std::ofstream &logger);
HMObjective
calculateObjectiveTTMDense(std::vector<HMInputParamBase *> &InputParams,
                           std::string matrix_name, std::ofstream &logger);
HMObjective calculateObjective(std::vector<HMInputParamBase *> &InParams,
                               std::string test_name, std::string matrix_name,
                               std::ofstream &logger);
...
std::string createjson(std::string AppName, std::string OutputFoldername,
                       int NumIterations, int NumDSERandomSamples,
                       std::vector<HMInputParamBase *> &InParams,
                       std::vector<std::string> Objectives, bool predictor,
                       std::string optimization, std::string count = "0") {
  ...
  HMScenario["log_file"] =
      OutputFoldername + "/log_" + AppName + "_" + count + ".log";
  if (optimization != "random_sampling" &&
      optimization != "embedding_random_sampling") {
  ...
  std::string JSonFileNameStr = CurrentDir + "/" + OutputFoldername + "/" +
                                AppName + "_" + optimization + "_scenario.json";
...
int collectInputParamsSpMV(std::vector<HMInputParamBase *> &InParams,
  ...
  HMInputParam<int> *chunkSizeParam =
      new HMInputParam<int>("chunk_size", ParamType::Integer);
  ...
  if (SPLIT) {
    ...
    HMInputParam<int> *splitParam =
        new HMInputParam<int>("split", ParamType::Categorical);
    splitParam->setRange(splitRange);
    InParams.push_back(splitParam);
    numParams++;

    std::vector<int> chunkSize2Range{2, 512};
    HMInputParam<int> *chunkSize2Param =
        new HMInputParam<int>("chunk_size2", ParamType::Integer);
  ...
  for (int i = 0; i < reorder_size; i++) {
    HMInputParam<int> *reorderParam =
        new HMInputParam<int>("loop" + std::to_string(i), ParamType::Integer);
...
int collectInputParamsSpMM(std::vector<HMInputParamBase *> &InParams) {
  ...
  std::vector<int> unrollFactorValues{1,  2,   4,   8,   16,  32,
                                      64, 128, 256, 512, 1024};
  ...
  HMInputParam<int> *chunkSizeParam =
      new HMInputParam<int>("chunk_size", ParamType::Ordinal);
  chunkSizeParam->setRange(chunkSizeValues);
  InParams.push_back(chunkSizeParam);
  numParams++;

  HMInputParam<int> *unrollFactorParam =
      new HMInputParam<int>("unroll_factor", ParamType::Ordinal);
  ...
  HMInputParam<int> *ompSchedulingTypeParam =
      new HMInputParam<int>("omp_scheduling_type", ParamType::Ordinal);
  ompSchedulingTypeParam->setRange(ompSchedulingType);
  InParams.push_back(ompSchedulingTypeParam);
  numParams++;

  HMInputParam<int> *ompChunkSizeParam =
      new HMInputParam<int>("omp_chunk_size", ParamType::Ordinal);
  ompChunkSizeParam->setRange(ompChunkSizeValues);
  InParams.push_back(ompChunkSizeParam);
  numParams++;

  HMInputParam<int> *ompNumThreadsParam =
      new HMInputParam<int>("omp_num_threads", ParamType::Ordinal);
  ...
  HMInputParam<std::vector<int>> *loopOrderingParam =
      new HMInputParam<std::vector<int>>("permutation", ParamType::Permutation);
...
int collectInputParamsTTV(std::vector<HMInputParamBase *> &InParams) {
  ...
  std::vector<int> chunkSizeFposValues{2,  4,   8,   16,  32,
                                       64, 128, 256, 512, 1024};
  ...
  HMInputParam<int> *chunkSizeIParam =
      new HMInputParam<int>("chunk_size_i", ParamType::Ordinal);
  chunkSizeIParam->setRange(chunkSizeIValues);
  InParams.push_back(chunkSizeIParam);
  numParams++;

  HMInputParam<int> *chunkSizeFposParam =
      new HMInputParam<int>("chunk_size_fpos", ParamType::Ordinal);
  chunkSizeFposParam->setRange(chunkSizeFposValues);
  InParams.push_back(chunkSizeFposParam);
  numParams++;

  HMInputParam<int> *chunkSizeKParam =
      new HMInputParam<int>("chunk_size_k", ParamType::Ordinal);
...
void setInputValue(HMInputParamBase *Param, std::string ParamVal) {
  ...
  case IntVector:
    ...
    static_cast<HMInputParam<std::vector<int>> *>(Param)->setVal(
        convertedVector);
    break;
  }
}
...
calculateObjectiveSpMVDense(std::vector<HMInputParamBase *> &InputParams,
  ...
  //   // std::cout << "in here\n";
  //   int order = static_cast<HMInputParam<int>*>(InputParams[i +
  //   1])->getVal(); loop_ordering.push_back(order);
  ...
  // bool default_config = (chunk_size == 16 && loop_ordering ==
  // default_ordering); spmv_handler->compute(default_config);
...
calculateObjectiveSpMMDense(std::vector<HMInputParamBase *> &InputParams,
  ...
  int chunk_size = static_cast<HMInputParam<int> *>(InputParams[0])->getVal();
  int unroll_factor =
      static_cast<HMInputParam<int> *>(InputParams[1])->getVal();
  int omp_scheduling_type =
      static_cast<HMInputParam<int> *>(InputParams[2])->getVal();
  int omp_chunk_size =
      static_cast<HMInputParam<int> *>(InputParams[3])->getVal();
  int omp_num_threads =
      static_cast<HMInputParam<int> *>(InputParams[4])->getVal();
  std::vector<int> loop_ordering =
      static_cast<HMInputParam<std::vector<int>> *>(InputParams[5])->getVal();
  ...
  if (!initialized) {
    ...
    taco::Tensor<double> temp_result({spmm_handler->NUM_I, spmm_handler->NUM_K},
                                     taco::dense);
    initialized = true;
  ...
  if (default_config_time == 0.0f) {
    ...
    spmm_handler->schedule_and_compute(
        temp_result, temp_chunk_size, temp_unroll_factor, temp_loop_ordering,
        temp_omp_scheduling_type, temp_omp_chunk_size, temp_omp_num_threads,
        false, 10);
    spmm_handler->set_cold_run();

    default_config_time = spmm_handler->get_compute_time();
    std::cout << spmm_handler->get_num_i() << "," << spmm_handler->get_num_j()
              << "," << default_config_time << "," << no_sched_time
              << std::endl;
    logger << spmm_handler->get_num_i() << "," << spmm_handler->get_num_j()
           << "," << default_config_time << "," << no_sched_time << std::endl;
  }

  if (!no_sched_init) {
    spmm_handler->schedule_and_compute(
        temp_result, chunk_size, unroll_factor, loop_ordering,
        omp_scheduling_type, omp_chunk_size, omp_num_threads, false, 10);
    spmm_handler->set_cold_run();
  }

  double compute_time =
      no_sched_init ? no_sched_time : spmm_handler->get_compute_time();
...
calculateObjectiveSDDMMDense(std::vector<HMInputParamBase *> &InputParams,
  ...
  // int chunk_size = static_cast<HMInputParam<int>*>(InputParams[0])->getVal();
  // int unroll_factor =
  // static_cast<HMInputParam<int>*>(InputParams[1])->getVal(); int
  // omp_chunk_size = static_cast<HMInputParam<int>*>(InputParams[2])->getVal();
  // int omp_scheduling_type =
  // static_cast<HMInputParam<int>*>(InputParams[3])->getVal(); std::vector<int>
  // loop_ordering =
  // static_cast<HMInputParam<std::vector<int>>*>(InputParams[4])->getVal(); int
  // num_threads = 32;
  int chunk_size = static_cast<HMInputParam<int> *>(InputParams[0])->getVal();
  int unroll_factor =
      static_cast<HMInputParam<int> *>(InputParams[1])->getVal();
  ...
  if (!initialized) {
    ...
    // sddmm_handler->generate_schedule(tmp_chunk_size, tmp_unroll_factor,
    // tmp_loop_ordering);
    compute_times = vector<double>();
    for (int i = 0; i < 5; i++) {
      double timer = 0.0;
  ...
  taco::Tensor<double> temp_result({sddmm_handler->NUM_I, sddmm_handler->NUM_J},
                                   taco::dense);
  ...
  if (default_config_time == 0.0f) {
    ...
    int temp_omp_chunk_size = 1;
    int temp_omp_num_threads = 32;
    sddmm_handler->schedule_and_compute(
        temp_result, temp_chunk_size, temp_unroll_factor, temp_loop_ordering,
        temp_omp_scheduling_type, temp_omp_chunk_size, temp_omp_num_threads,
        false, 10);
    ...
    std::cout << sddmm_handler->get_num_i() << "," << sddmm_handler->get_num_j()
              << "," << default_config_time << "," << no_sched_time
              << std::endl;
    logger << sddmm_handler->get_num_i() << "," << sddmm_handler->get_num_j()
           << "," << default_config_time << "," << no_sched_time << std::endl;
  }
  ...
  double compute_time =
      no_sched_init ? no_sched_time : sddmm_handler->get_compute_time();
  Obj.compute_time = compute_time;
  return Obj;
}
...
calculateObjectiveTTVDense(std::vector<HMInputParamBase *> &InputParams,
  ...
  int omp_chunk_size =
      static_cast<HMInputParam<int> *>(InputParams[4])->getVal();
  int omp_num_threads =
      static_cast<HMInputParam<int> *>(InputParams[5])->getVal();
  std::vector<int> loop_ordering =
      static_cast<HMInputParam<std::vector<int>> *>(InputParams[6])->getVal();
  std::vector<int> default_ordering{0, 1, 2, 3, 4};
  ...
  taco::Tensor<double> temp_result({ttv_handler->NUM_I, ttv_handler->NUM_J},
                                   taco::dense);
  ...
  if (default_config_time == 0.0f) {
    ...
    ttv_handler->schedule_and_compute(
        temp_result, temp_chunk_size_i, temp_chunk_size_fpos, temp_chunk_size_k,
        temp_loop_ordering, temp_omp_scheduling_type, temp_omp_chunk_size,
        temp_omp_num_threads, false);
    ttv_handler->set_cold_run();

    default_config_time = ttv_handler->get_compute_time();
    logger << ttv_handler->get_num_i() << "," << ttv_handler->get_num_j() << ","
           << default_config_time << "," << no_sched_time << std::endl;
  }

  if (!no_sched_init) {
    try {
      ttv_handler->schedule_and_compute(temp_result, chunk_size_i,
                                        chunk_size_fpos, chunk_size_k,
                                        loop_ordering, omp_scheduling_type,
                                        omp_chunk_size, omp_num_threads, false);
      ttv_handler->set_cold_run();
...
calculateObjectiveTTMDense(std::vector<HMInputParamBase *> &InputParams,
  ...
  taco::Tensor<double> temp_result(
      {ttm_handler->NUM_I, ttm_handler->NUM_J, ttm_handler->NUM_L},
      taco::dense);
  ...
  if (default_config_time == 0.0f) {
    ...
    int temp_omp_chunk_size = 1;
    int temp_omp_num_threads = 32;
    ttm_handler->schedule_and_compute(
        temp_result, temp_chunk_size, temp_unroll_factor, temp_loop_ordering,
        temp_omp_scheduling_type, temp_omp_chunk_size, temp_omp_num_threads,
        false);
    ttm_handler->set_cold_run();
  ...
  if (!no_sched_init) {
    try {
      ttm_handler->schedule_and_compute(temp_result, chunk_size, unroll_factor,
                                        loop_ordering, omp_scheduling_type,
                                        omp_chunk_size, omp_num_threads, false);
      ttm_handler->set_cold_run();
...
HMObjective
calculateObjectiveMTTKRPDense(std::vector<HMInputParamBase *> &InputParams,
                              std::string matrix_name, std::ofstream &logger) {
  using namespace taco;
  HMObjective Obj;
  int chunk_size = static_cast<HMInputParam<int> *>(InputParams[0])->getVal();
  ...
  compute_times = vector<double>();
  taco::Tensor<double> temp_result(
      {mttkrp_handler->NUM_I, mttkrp_handler->NUM_J}, taco::dense);

  // mttkrp_handler->schedule_and_compute(temp_result, chunk_size,
  // unroll_factor, loop_ordering, omp_scheduling_type, omp_chunk_size,
  ...
  if (default_config_time == 0.0f) {
    ...
    int temp_omp_num_threads = 32;
    // default_config_time = ttv_handler->get_default_compute_time();
    mttkrp_handler->schedule_and_compute(
        temp_result, temp_chunk_size, temp_unroll_factor, temp_loop_ordering,
        temp_omp_scheduling_type, temp_omp_chunk_size, temp_omp_num_threads,
        false);
  ...
  if (!no_sched_init) {
    try {
      mttkrp_handler->schedule_and_compute(
          temp_result, chunk_size, unroll_factor, loop_ordering,
          omp_scheduling_type, omp_chunk_size, omp_num_threads, false);
      mttkrp_handler->set_cold_run();
...
int single_run_spmm(std::string matrix_name, int chunk_size, int unroll_factor,
  ...
  taco::Tensor<double> temp_result({spmm_handler->NUM_I, spmm_handler->NUM_K},
                                   taco::dense);
  for (int i = 0; i < 5; i++) {
    // spmm_handler->schedule_and_compute(temp_result, chunk_size,
    // unroll_factor, default_ordering, omp_scheduling_type, omp_chunk_size,
    // omp_num_threads, false);
...
int main(int argc, char **argv) {
  ...
  if (single_run) {
    ...
    int omp_num_threads = 32;
    single_run_spmm(matrix_name, chunk_size, unroll_factor, omp_scheduling_type,
                    omp_chunk_size, omp_num_threads);
  ...
  OutputFoldernameMat =
      ExperimentFolder + "/outdata_" + test_name + "_" + rawname;
  ...
  if (fs::exists(OutputDir)) {
    ...
    // exit(1);
    std::string csv_file = OutputFoldername + "/" + AppName + "_" +
                           optimization + count + "_output_data.csv";
    ...
    // std::string last_csv_file = OutputFoldername + "/" + AppName + "_" +
    // optimization + "9" + "_output_data.csv"; if(fs::exists(csv_file) &&
    // !fs::exists(last_csv_file) && fs::exists(png_file)) {
    //   std::cerr << "CSV file and png file exists, exiting";
    //   exit(0);
    // }
    // if(fs::exists(csv_file) && fs::exists(last_csv_file) &&
    // fs::exists(png_file)) {
  ...
  JSonFileNameStr = createjson(
      AppName, OutputFoldername + "/" + OutputSubFolderName, NumIterations,
  ...
  while (true) {
    ...
    for (int request = 0; request < numRequests; request++) {
      ...
      HMObjective Obj =
          calculateObjective(InParams, test_name, matrix_name,
                             logger); // Function to run hypermapper on
  ...
  cmdPareto += " -i " + OutputFoldername + " --plot_log -o " +
               OutputFoldername + "/" + test_name + "_plot.png";
  cmdPareto += " --expert_configuration " + to_string(default_config_time);
  cmdPareto += " -t '" + op + " " + to_string(num_i) + "x" + to_string(num_j) +
               " d:" + to_string(dimensionality_plus_one - 1) +
               " sparsity:" + to_string(sparsity) + "'";
  cmdPareto += " -doe ";

  std::string title = op + " " + to_string(num_i) + "x" + to_string(num_j) +
                      " d:" + to_string(dimensionality_plus_one - 1) +
                      " sparsity:" + to_string(sparsity);
