...
struct UfuncInputCache {
  taco::Tensor<double> get_tensor(int num_k = 1000, float SPARSITY = 0.3) {
    taco::Tensor<double> tensor(
        "tensor", {this->num_j, num_k},
        taco::Format{taco::ModeFormat::Dense, taco::ModeFormat::Dense});
    srand(120);
  ...
  template <typename U> void loadTensor(std::string path, U format) {
    if (this->lastPath != path) {
      // Otherwise, we missed the cache. Load in the target tensor and process
      // it.
      this->lastLoaded = taco::read(path, format);
      // We assign lastPath after lastLoaded so that if taco::read throws an
      // exception then lastPath isn't updated to the new path.
      this->lastPath = path;
...
#else
    for (auto val = this->inputTensor.template beginTyped<int64_t>();
         val != this->inputTensor.template endTyped<int64_t>(); ++val) {
#endif
      this->nnz++;
    }
  }
  ...
  template <typename U>
  taco::Tensor<double> getMat(std::string path, U format, bool countNNZ = false,
  ...
  template <typename U>
  std::pair<taco::Tensor<double>, taco::Tensor<double>>
  getUfuncInput(std::string path, U format, bool countNNZ = false,
                float sparsity = 0.3, int num_k = 1000,
                bool includeThird = false) {
    loadTensor(path, format);
  ...
  template <typename U>
  taco::Tensor<double> getTensor(std::string path, U format,
                                 bool shift_dim = false, bool countNNZ = false,
                                 float sparsity = 0.3, int num_k = 1000,
                                 bool includeThird = false) {
    loadTensor(path, format);
    this->inputTensor.setName("loaded");
    ...
    taco::Tensor<double> copy("test", {this->num_i, this->num_k, last_dim},
                              taco::Sparse);
...
public:
  ...
  std::pair<taco::Tensor<double>, taco::Tensor<double>>
  load_tensor(std::string tensorPath, int num_k = 1000, float sparsity = 0.3) {
    auto suitesparsePath = std::getenv("SUITESPARSE_PATH");
    if (suitesparsePath == nullptr) {
      std::cout << "Set SUITESPARSE_PATH" << std::endl;
      exit(1);
    }
    return inputCache.getUfuncInput(tensorPath, taco::CSR, true, sparsity,
                                    num_k);
  }
  ...
  void get_reordering(std::vector<taco::IndexVar> &reordering_out,
...
public:
  ...
  int run_mode;
  SpMV(int mode, int NUM_I = 10000, int NUM_J = 10000, float SPARSITY = .3)
      : NUM_I{NUM_I}, NUM_J{NUM_J}, SPARSITY{SPARSITY}, initialized{false},
        reorder_initialized{false}, cold_run{true},
        B("B", {NUM_I, NUM_J}, taco::CSR),
        c("c", {NUM_J}, taco::Format{taco::ModeFormat::Dense}),
        a("a", {NUM_I}, taco::Format{taco::ModeFormat::Dense}), i0("i0"),
        i1("i1"), i10("i10"), i11("i11"), kpos("kpos"), kpos0("kpos0"),
        kpos1("kpos1") {}
  SpMV()
      : run_mode(1), initialized{false}, reorder_initialized{false},
        cold_run{true}, i0("i0"), i1("i1"), i10("i10"), i11("i11"), j0("j0"),
        j1("j1"), kpos("kpos"), kpos0("kpos0"), kpos1("kpos1") {}
  ...
  float get_sparsity() {
    return (run_mode == 0) ? SPARSITY : inputCache.get_sparsity();
  }
  int get_num_i() { return NUM_I; }
  int get_num_j() { return NUM_J; }

  taco::IndexStmt schedule(taco::IndexStmt &sched, std::vector<int> order,
                           int CHUNK_SIZE = 16, int CHUNK_SIZE2 = 8,
                           int CHUNK_SIZE3 = 8, int omp_scheduling_type = 0,
                           int omp_chunk_size = 1, int num_threads = 32) {
    using namespace taco;
    ...
    if (omp_scheduling_type == 0) {
      taco::taco_set_parallel_schedule(taco::ParallelSchedule::Static,
                                       omp_chunk_size);
    } else if (omp_scheduling_type == 1) {
      taco::taco_set_parallel_schedule(taco::ParallelSchedule::Dynamic,
                                       omp_chunk_size);
    }
    return sched.split(i, i0, i1, CHUNK_SIZE)
        ...
        .parallelize(j1, ParallelUnit::CPUVector,
                     OutputRaceStrategy::IgnoreRaces);
  }

  taco::IndexStmt schedule_(std::vector<int> order, int CHUNK_SIZE = 16,
                            int SPLIT = 0, int CHUNK_SIZE2 = 8) {
    using namespace taco;
    if (SPLIT) {
      std::vector<taco::IndexVar> reorder = get_reordering(order);
  ...
  taco::IndexStmt schedule2(int CHUNK_SIZE = 16, int SPLIT = 0,
                            int CHUNK_SIZE2 = 8, int order = 0) {
    using namespace taco;
    if (SPLIT) {
      std::vector<taco::IndexVar> reorder = get_reordering(order);
      return stmt.split(i, i0, i1, CHUNK_SIZE)
  ...
  taco::IndexStmt schedule(int CHUNK_SIZE = 16, int SPLIT = 0,
                           int CHUNK_SIZE2 = 8, int order = 0) {
    using namespace taco;
    if (SPLIT) {
      std::vector<taco::IndexVar> reorder = get_reordering(order);
  ...
  void generate_schedule(int chunk_size, int split, int chunk_size2,
  ...
  void schedule_and_compute(taco::Tensor<double> &result, int chunk_size,
                            int chunk_size2, int chunk_size3,
                            std::vector<int> order, int omp_scheduling_type = 0,
                            int omp_chunk_size = 0, int num_threads = 32,
                            bool default_config = false, int num_reps = 20) {
    result(i) = B(i, j) * c(j);
    std::cout << "computing\n";
    taco::IndexStmt sched = result.getAssignment().concretize();

    sched = schedule(sched, order, chunk_size, chunk_size2, chunk_size3,
                     omp_scheduling_type, omp_chunk_size, num_threads);
...
public:
  ...
  SpMM(int mode, int NUM_I = 10000, int NUM_J = 10000, int NUM_K = 1000,
       float SPARSITY = .3)
      : run_mode(0), NUM_I{NUM_I}, NUM_J{NUM_J}, NUM_K{NUM_K},
        SPARSITY{SPARSITY}, initialized{false}, cold_run{true},
        A("A", {NUM_I, NUM_K},
          taco::Format{taco::ModeFormat::Dense, taco::ModeFormat::Dense}),
        B("B", {NUM_I, NUM_J}, taco::CSR),
        C("C", {NUM_J, NUM_K},
          taco::Format{taco::ModeFormat::Dense, taco::ModeFormat::Dense}),
        expected(
            "expected", {NUM_I, NUM_K},
            taco::Format{taco::ModeFormat::Dense, taco::ModeFormat::Dense}),
        i0("i0"), i1("i1"), kbounded("kbounded"), k0("k0"), k1("k1"),
        jpos("jpos"), jpos0("jpos0"), jpos1("jpos1"), j0("j0"), j1("j1") {}
  SpMM()
      : run_mode(1), initialized{false}, cold_run{true}, i0("i0"), i1("i1"),
        kbounded("kbounded"), k0("k0"), k1("k1"), jpos("jpos"), jpos0("jpos0"),
        jpos1("jpos1"), j0("j0"), j1("j1") {}
  ...
  taco::IndexStmt schedule(std::vector<int> order, int chunk_size = 16,
                           int unroll_factor = 8, int omp_scheduling_type = 0,
                           int omp_chunk_size = 1, int num_threads = 32) {
    using namespace taco;
    std::vector<taco::IndexVar> reorder; //= get_reordering(order);
    reorder.reserve(order.size());
    ...
    return stmt.split(i, i0, i1, chunk_size)
        ...
        .reorder(reorder)
        .parallelize(i0, ParallelUnit::CPUThread, OutputRaceStrategy::NoRaces)
        .parallelize(k, ParallelUnit::CPUVector,
                     OutputRaceStrategy::IgnoreRaces);
  }

  taco::IndexStmt schedule(taco::IndexStmt &sched, std::vector<int> order,
                           int chunk_size = 16, int unroll_factor = 8,
                           int omp_scheduling_type = 0, int omp_chunk_size = 1,
                           int num_threads = 32) {
    using namespace taco;
    std::vector<taco::IndexVar> reorder; //= get_reordering(order);
    reorder.reserve(order.size());
  ...
  taco::IndexStmt schedule2(std::vector<int> order, int chunk_size = 16,
                            int unroll_factor = 8, int omp_scheduling_type = 0,
                            int omp_chunk_size = 1, int num_threads = 32) {
    using namespace taco;
    std::vector<taco::IndexVar> reorder; //= get_reordering(order);
    ...
    return stmt.split(i, i0, i1, chunk_size)
        ...
        .reorder(reorder)
        .parallelize(i0, ParallelUnit::CPUThread, OutputRaceStrategy::NoRaces)
        .parallelize(jpos1, ParallelUnit::CPUVector,
                     OutputRaceStrategy::ParallelReduction);
  }
  taco::IndexStmt schedule(int chunk_size = 16, int unroll_factor = 8,
                           int order = 0) {
    using namespace taco;
    std::vector<taco::IndexVar> reorder = get_reordering(order);
    return stmt.split(i, i0, i1, chunk_size)
  ...
  void generate_schedule(taco::Tensor<double> &result, int chunk_size,
                         int unroll_factor, std::vector<int> order,
                         int omp_scheduling_type, int omp_chunk_size,
                         int num_threads) {
    result(i, k) = B(i, j) * C(j, k);

    stmt = result.getAssignment().concretize();
    stmt = schedule(order, chunk_size, unroll_factor, omp_scheduling_type,
                    omp_chunk_size, num_threads);
  }
  ...
  taco::IndexStmt schedule3(taco::IndexStmt &sched, std::vector<int> order,
                            int chunk_size = 16, int unroll_factor = 8,
                            int omp_scheduling_type = 0, int omp_chunk_size = 1,
                            int num_threads = 32) {
    using namespace taco;
  ...
  void schedule_and_compute(taco::Tensor<double> &result, int chunk_size,
                            int unroll_factor, std::vector<int> order,
                            int omp_scheduling_type = 0, int omp_chunk_size = 0,
                            int num_threads = 32, bool default_config = false,
                            int num_reps = 20) {
    result(i, k) = B(i, j) * C(j, k);

    taco::IndexStmt sched = result.getAssignment().concretize();

    sched = schedule(sched, order, chunk_size, unroll_factor,
                     omp_scheduling_type, omp_chunk_size, num_threads);
...
public:
  ...
  int get_num_i() { return NUM_I; }
  int get_num_j() { return NUM_J; }
  SDDMM(int mode, int NUM_I = 10000, int NUM_J = 10000, int NUM_K = 1000,
        float SPARSITY = .3)
      : NUM_I{NUM_I}, NUM_J{NUM_J}, NUM_K{NUM_K}, SPARSITY{SPARSITY},
        initialized{false}, cold_run{true},
        A("A", {NUM_I, NUM_J},
          taco::Format{taco::ModeFormat::Dense, taco::ModeFormat::Dense}),
        B("B", {NUM_I, NUM_J}, taco::CSR),
        C("C", {NUM_I, NUM_K},
  ...
  void initialize_data(int mode = RANDOM) override {
    ...
    cout << "A: [" << A.getDimensions()[0] << "," << A.getDimensions()[1] << "]"
         << endl;
    cout << "B: [" << B.getDimensions()[0] << "," << B.getDimensions()[1] << "]"
         << endl;
    cout << "C: [" << C.getDimensions()[0] << "," << C.getDimensions()[1] << "]"
         << endl;
    cout << "D: [" << D.getDimensions()[0] << "," << D.getDimensions()[1] << "]"
         << endl;
  ...
  void generate_schedule(std::vector<int> order, int chunk_size,
                         int unroll_factor) {
    A(i, j) = B(i, j) * C(i, k) * D(k, j);
  ...
  void generate_schedule(int chunk_size, int unroll_factor,
                         std::vector<int> order, int omp_scheduling_type = 0,
                         int omp_chunk_size = 0, int num_threads = 32) {
    A(i, j) = B(i, j) * C(i, k) * D(k, j);
    taco::taco_set_num_threads(num_threads);
...
public:
  ...
  TTV(int mode, int NUM_I = 1000, int NUM_J = 1000, int NUM_K = 1000,
      float SPARSITY = .3)
      : NUM_I{NUM_I}, NUM_J{NUM_J}, NUM_K{NUM_K}, SPARSITY{SPARSITY},
        initialized{false}, cold_run{true},
        A("A", {NUM_I, NUM_J},
          taco::Format{taco::ModeFormat::Dense, taco::ModeFormat::Dense}),
        B("B", {NUM_I, NUM_J, NUM_K},
          taco::Format{taco::ModeFormat::Sparse, taco::ModeFormat::Sparse,
                       taco::ModeFormat::Sparse}),
        c("c", {NUM_K}, taco::Format{taco::ModeFormat::Dense}), f("f"),
        fpos("fpos"), chunk("chunk"), fpos2("fpos2"), k1("k1"), k2("k2"),
        i0("i0"), i1("i1") {}
  TTV()
      : run_mode(1), initialized{false}, cold_run{true}, f("f"), fpos("fpos"),
        chunk("chunk"), fpos2("fpos2"), k1("k1"), k2("k2"), i0("i0"), i1("i1"),
  ...
  void initialize_data(int mode = RANDOM) override {
    ...
    std::cout << "Dimensions: " << NUM_I << ", " << NUM_J << ", " << NUM_K
              << std::endl;
    std::cout << "NNZ: " << nnz << std::endl;

    taco::Tensor<double> c_("c", {NUM_K},
                            taco::Format{taco::ModeFormat::Dense});
    c = c_;
    for (int k = 0; k < NUM_K; k++) {
      float rand_float = (float)rand() / (float)(RAND_MAX);
  ...
  taco::IndexStmt schedule(int CHUNK_SIZE = 16, int order = 0) {
    ...
    return stmt.fuse(i, j, f)
        ...
        .reorder(reorder)
        .parallelize(chunk, ParallelUnit::CPUThread,
                     OutputRaceStrategy::NoRaces);
  }
  ...
  double compute_unscheduled() {
    taco::Tensor<double> result = copyNonZeroStructure(
        {NUM_I, NUM_J}, {taco::Sparse, taco::Sparse}, B, 2);
  ...
  taco::IndexStmt schedule(taco::IndexStmt &sched, std::vector<int> order,
                           int chunk_size_i = 16, int chunk_size_fpos = 16,
                           int chunk_size_k = 16, int omp_scheduling_type = 0,
                           int omp_chunk_size = 0, int omp_num_threads = 32) {
    ...
    get_reordering(reorder, order);
    return sched.split(i, i0, i1, chunk_size_i)
        .fuse(i1, j, f)
        .pos(f, fpos, B(i, j, k))
        ...
        .parallelize(chunk, ParallelUnit::CPUThread,
                     OutputRaceStrategy::NoRaces)
        .parallelize(kpos2, ParallelUnit::CPUVector,
                     OutputRaceStrategy::ParallelReduction);
  }

  void schedule_and_compute(taco::Tensor<double> &result_, int chunk_size_i,
                            int chunk_size_fpos, int chunk_size_k,
                            std::vector<int> order, int omp_scheduling_type = 0,
                            int omp_chunk_size = 0, int num_threads = 32,
    ...
    sched = schedule(sched, order, chunk_size_i, chunk_size_fpos, chunk_size_k,
                     omp_scheduling_type, omp_chunk_size, num_threads);
    cold_run = false;
...
public:
  ...
  TTM()
      : run_mode(1), initialized{false}, cold_run{true}, f("f"), fpos("fpos"),
        chunk("chunk"), fpos2("fpos2"), kpos("kpos"), kpos1("kpos1"),
        kpos2("kpos2") {}
  TTM(int mode, int NUM_I = 1000, int NUM_J = 1000, int NUM_K = 1000,
      int NUM_L = 1000, float SPARSITY = .1)
      : NUM_I{NUM_I}, NUM_J{NUM_J}, NUM_K{NUM_K}, NUM_L{NUM_L},
        SPARSITY{SPARSITY}, initialized{false}, cold_run{true},
        A("A", {NUM_I, NUM_J}, taco::CSR),
        B("B", {NUM_J, NUM_K},
          {taco::ModeFormat::Dense, taco::ModeFormat::Dense}),
        C("C", {NUM_I, NUM_K},
          {taco::ModeFormat::Dense, taco::ModeFormat::Dense}),
        f("f"), fpos("fpos"), chunk("chunk"), fpos2("fpos2"), kpos("kpos"),
        kpos1("kpos1"), kpos2("kpos2") {}
  void set_cold_run() { cold_run = true; }
  void initialize_data(int mode = RANDOM) override {
    // TODO: Implement read from matrix market mode
    using namespace taco;
    ...
    taco::Tensor<double> C_("C", {NUM_K, NUM_L},
                            {taco::ModeFormat::Dense, taco::ModeFormat::Dense});
    C = C_;
    for (int k = 0; k < NUM_K; k++) {
  ...
  double compute_unscheduled() {
    taco::Tensor<double> result = copyNonZeroStructure(
        {NUM_I, NUM_J, NUM_L}, {taco::Sparse, taco::Sparse, taco::Dense}, B, 2);
  ...
  void schedule_and_compute(taco::Tensor<double> &result_, int chunk_size,
                            int unroll_factor, std::vector<int> order,
                            int omp_scheduling_type = 0, int omp_chunk_size = 0,
                            int num_threads = 32, bool default_config = false,
                            int num_reps = 20) {
...
public:
  ...
  int run_mode;
  MTTKRP()
      : run_mode(1), initialized{false}, cold_run{true}, i("i"), i1("i1"),
        i2("i2"), k("k"), l("l"), m("m"), j("j") {}
  MTTKRP(int mode, int NUM_I = 1000, int NUM_J = 1000, int NUM_K = 1000,
         int NUM_L = 1000, int NUM_M = 1000, float SPARSITY = .1)
      : NUM_I{NUM_I}, NUM_J{NUM_J}, NUM_K{NUM_K}, NUM_L{NUM_L},
        SPARSITY{SPARSITY}, initialized{false}, cold_run{true},
        A("A", {NUM_I, NUM_J}, {taco::Dense, taco::Dense}),
        B("B", {NUM_I, NUM_K, NUM_L, NUM_M},
          {taco::Dense, taco::Sparse, taco::Sparse, taco::Sparse}),
        C("C", {NUM_K, NUM_J}, {taco::Dense, taco::Dense}),
        D("D", {NUM_L, NUM_J}, {taco::Dense, taco::Dense}),
        E("E", {NUM_M, NUM_J}, {taco::Dense, taco::Dense}), i("i"), i1("i1"),
        i2("i2"), k("k"), l("l"), m("m"), j("j") {}
  void set_cold_run() { cold_run = true; }
  void initialize_data(int mode = RANDOM) override {
    // TODO: Implement read from matrix market mode
    ...
    std::cout << "Size: " << NUM_I << ", " << NUM_K << ", " << NUM_L << ", "
              << NUM_M << ", " << NUM_J << std::endl;

    taco::Tensor<double> C_("C", {NUM_K, NUM_J},
                            {taco::ModeFormat::Dense, taco::ModeFormat::Dense});
    taco::Tensor<double> D_("D", {NUM_L, NUM_J},
                            {taco::ModeFormat::Dense, taco::ModeFormat::Dense});
    taco::Tensor<double> E_("E", {NUM_M, NUM_J},
                            {taco::ModeFormat::Dense, taco::ModeFormat::Dense});
    C = C_;
    D = D_;
    E = E_;
