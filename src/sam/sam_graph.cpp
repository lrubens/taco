...
struct SAMGraph::Content {
    Content(IterationForest iterationForest, const vector<IndexVar> &freeVars,
        ...
        : iterationForest(iterationForest),
          ...
          accessNodesToPaths(mapAccessNodesToPaths), expr(expr) {}
    IterationForest iterationForest;
...
SAMGraph SAMGraph::make(Assignment assignment) {
    ...
    match(expr, function<void(const AccessNode *)>([&](const AccessNode *op) {
              ...
              taco_iassert((size_t)type.getShape().getOrder() ==
                           op->indexVars.size())
                  << "Tensor access " << IndexExpr(op)
                  << " but tensor format only has "
    ...
    samGraph.content = make_shared<SAMGraph::Content>(
        forest, freeVars, resultPath, tensorPaths, accessNodesToPaths, expr);
    return samGraph;
}
...
const std::vector<IndexVar> &SAMGraph::getChildren(const IndexVar &var) const {
...
const TensorPath &SAMGraph::getTensorPath(const IndexExpr &operand) const {
...
IndexVarType SAMGraph::getIndexVarType(const IndexVar &var) const {
    return (util::contains(content->freeVars, var)) ? IndexVarType::Free
                                                    : IndexVarType::Sum;
}
...
bool SAMGraph::isReduction(const IndexVar &var) const { return !isFree(var); }
    ...
    if (roots.size() > 0) {
        ...
        while (count < (int)indexVars.size()) {
            ...
            if (content->iterationForest.hasChildren(indexvar)) {
                ...
                indexVars.insert(indexVars.end(), children.begin(),
                                 children.end());
            }
            count++;
        }
...
map<IndexVar, ModeFormat> SAMGraph::getFormatMapping(const TensorPath path) {
    ...
    taco_iassert(tensor.getFormat().getModeOrdering().size() ==
                 path.getVariables().size());
...
size_t SAMGraph::getMode(const IndexVar &indexvar,
                         const TensorPath &path) const {
    auto vars = path.getVariables();
    auto tensor = path.getAccess().getTensorVar();
    auto it = find(vars.begin(), vars.end(), indexvar);
    taco_iassert(it != vars.end())
        << "Indexvar " << indexvar << " should be in the tensor path" << path;
    auto mode =
        tensor.getFormat().getModeOrdering().at(distance(vars.begin(), it));
    return mode;
}

vector<IndexVar> getAccessVars(IndexExpr expr) {
    set<IndexVar> ivars;
    match(expr, function<void(const taco::AccessNode *, Matcher *)>(
                    [&](const taco::AccessNode *op, Matcher *ctx) {
                        auto tensor = op->tensorVar;
                        set<IndexVar> temp(op->indexVars.begin(),
                                           op->indexVars.end());
...
int getNumReductionNodes(IndexExpr expr) {
    int result = 0;
    match(expr, function<void(const taco::ReductionNode *, Matcher *)>(
...
struct RemoveReductionTree : public IndexNotationRewriter {
    ...
    IndexExpr removeReductionTree(IndexExpr expr) { return rewrite(expr); }
...
SamIR SAMGraph::makeGraph() {
    ...
    // Generally it defaults to all index variables in expression, but this is
    // not true for expressions with inner ReductionNodes (sums)
    map<TensorVar, vector<IndexVar>> inputIterationIndexVarMap;
    // By default all tensors broadcast to all index variables
    // Just remove indexvars used in reductions from the tensors OUTSIDE of the
    // reduction I.e. b(i) - sum(j, C(i,j)*d(j) should produce the map {b:i,
    // C:i,j, d:i,j}
    IndexExpr tempExpr = content->expr;
    for (int ii = 0; ii < getNumReductionNodes(content->expr); ii++) {
        match(tempExpr, function<void(const taco::ReductionNode *, Matcher *)>(
                            [&](const taco::ReductionNode *op, Matcher *ctx) {
                                if (getNumReductionNodes(op->a) == 0) {
                                    auto tensors = getTensorVars(op->a);
                                    auto indexVars = getAccessVars(op->a);
                                    for (auto tensor : tensors) {
                                        inputIterationIndexVarMap[tensor] =
                                            indexVars;
                                    }
                                }
                                ctx->match(op->a);
    ...
    for (const auto &indexvar : getOrderedIndexVars()) {
        if (!contains(dimensionMap, indexvar)) {
            ...
            dimensionMap[indexvar] =
                resultTensor.getName() + to_string(mode) + "_dim";
        }
    }

    // Create map from index variable to datastructure sizes using dimensionMap
    // above A compressed level will always have sizes:
    ...
    for (int count = 0; count < (int)getResultTensorPath().getSize(); count++) {
        ...
        if (count == 0) {
            ...
            crdSize = sizeMap.at(prevIndexVar).second + "*" +
                      dimensionMap.at(indexvar);
        }
    ...
    SamIR resultWriteVals =
        FiberWrite(nullptr, getResultTensorPath().getAccess().getTensorVar(),
                   -1, "", valsSizeStr, id, true, true);
    id++;
    ...
    match(content->expr,
          function<void(const taco::MulNode *, Matcher *)>(
              [&](const taco::MulNode *op, Matcher *ctx) {
                  ...
                  for (auto &lhsVar : lhsVars) {
                      for (auto &rhsVar : rhsVars) {
                          if (lhsVar.first == rhsVar.first) {
                              vector<TensorVar> tensors(lhsVar.second.size() +
                                                        rhsVar.second.size());
                              merge(lhsVar.second.begin(), lhsVar.second.end(),
                                    rhsVar.second.begin(), rhsVar.second.end(),
                                    tensors.begin());
                  ...
                  if (parentRHS) {
                      rhsAccess = thisRHS;
                      rhsAccess.insert(rhsAccess.end(),
                                       std::make_move_iterator(thisLHS.begin()),
                                       std::make_move_iterator(thisLHS.end()));
                  } else {
                      lhsAccess = thisLHS;
                      lhsAccess.insert(lhsAccess.end(),
                                       std::make_move_iterator(thisRHS.begin()),
    ...
    // Make map of index variables to a list of the tensor variables involved in
    ...
    for (int count = 0; count < numIndexVars; count++) {
        ...
        if (std::count(resultVars.begin(), resultVars.end(), indexvar) > 0) {
            ...
            auto node = FiberWrite(
                indexvar, getResultTensorPath().getAccess().getTensorVar(),
                (int)mode, sizeStr.first, sizeStr.second, id, true);
            id++;
    ...
    if (!reduction.empty()) {
        ...
        //            for (auto it =  reduction.rbegin(); it !=
        //            reduction.rend(); it++) {
        //                auto red = *it;
        //                switch (red) {
        ...
        //                        "Reduce node must have a reduction order of
        //                        0"; taco_iassert(!reductionOrder.empty()) <<
        //                        "Number of reduction (Reduction) nodes does
        //                        not "
        //                                                                 "match
        //                                                                 the
        //                                                                 number
        ...
        //                        taco_iassert(!reductionOrder.empty()) <<
        //                        "Number of reduction (Sparse Accumulation)
        //                        nodes does not
        //                        "
        //                                                                 "match
        //                                                                 the
        ...
        //                                                                 orders.";
        //
        ////                    for (int i = numIndexVars - 1; i >=
        ///(numIndexVars
        ///-
        /// 1 - reductionOrder.back()); i--) { /                            auto
        /// indexvar = getOrderedIndexVars().at(i); / // Only need to add in
        /// output coordinates to the sparse accumulator if there / // is a
        /// level writer / if
        ///(contains(resultWriteIRNodes, indexvar)) { / spAccCrds[spaccInputCnt]
        ///=
        /// resultWriteIRNodes[indexvar]; /                                //
        /// Only needed for CrdHold generation / // outermostSpAccVar = i - 1; /
        /// // innermostSpAccVar = innermostSpAccVar < 0 ? i-1 :
        /// innermostSpAccVar; /
        /// }
        ...
        /// i--) { /                            // FIXME: check if this is
        /// always correct for more complicated kernels / auto indexvar =
        /// getOrderedIndexVars().at(i); / cout << indexvar << ", " << endl; /
        /// // if
    ...
    //        SamIR crdhold;
    //        if (std::count(reduction.begin(), reduction.end(),
    //        SamNodeType::SparseAccumulator)) {
    //            for (int count = innermostSpAccVar - 1; count >=
    //            outermostSpAccVar; count--) {
    ...
    match(
        ...
        function<void(const taco::AccessNode *, Matcher *)>(
            [&](const taco::AccessNode *op, Matcher *ctx) {
                ...
                auto array =
                    taco::sam::Array(computeBlock, tensor, id, false,
                                     isScalar && getOrderedIndexVars().empty());
                id++;
        ...
        function<void(const taco::ReductionNode *, Matcher *)>(
            [&](const taco::ReductionNode *op, Matcher *ctx) {
                ...
                if (op->var == getOrderedIndexVars().at(numIndexVars - 1) ||
                    ...
                    reduce = taco::sam::SparseAccumulator(
                        prevComputeNode, spAccCrds, reductionOrder.back(),
                        spAccIndexvarMap, id);

                    // Only pass coordinates to sparse accumulators for sparse
                    // accumulator d + 1, where d is the dimension of the sparse
                    // accumulator. For example, a sparse accumulator for a
                    // vector should take both the value and
                    for (int i = numIndexVars - 1;
                         i >= (numIndexVars - 1 - reductionOrder.back()); i--) {
    ...
    // ThIS IS THE OLD CRDDROP ALGORITHM WHICH IS PROBABLY WRONG (Based only on
    // intersections with adjacent intersection levels)
    //        bool adjacentContractionLevel = false;
    //        IndexVar prevContractionVar = IndexVar();
    //        bool outerIntersection = false;
    //        for (int count = 0; count < (int) getOrderedIndexVars().size();
    //        count++) {
    //            IndexVar indexVar = getOrderedIndexVars().at(count);
    //
    //            bool isIntersection = contains(contractionType,
    //            contractions.at(indexVar)) &&
    ...
    //                auto node =
    //                CrdDrop(inputIterationCrdDst[prevContractionVar],
    //                inputIterationCrdDst[indexVar],
    ...
    for (int count = 0; count < (int)getOrderedIndexVars().size(); count++) {
        ...
        for (int outerCount = 0; outerCount < count; outerCount++) {
            ...
            bool isOuterResult =
                std::count(resultVars.begin(), resultVars.end(),
                           outerIndexVar) > 0;
        ...
        for (int innerCount = count;
             innerCount < (int)getOrderedIndexVars().size(); innerCount++) {
            IndexVar innerIndexVar = getOrderedIndexVars().at(innerCount);
            bool isIntersection =
                contains(contractionType, contractions.at(innerIndexVar)) &&
                contractionType.at(contractions.at(innerIndexVar));
            innerIntersection = innerIntersection || isIntersection;
        }

        bool isResult =
            std::count(resultVars.begin(), resultVars.end(), indexVar) > 0;

        if (innerIntersection and hasOuterResult) {
            auto node = CrdDrop(inputIterationCrdDst[prevContractionVar],
                                inputIterationCrdDst[indexVar],
                                prevContractionVar, indexVar, id);
            id++;
    ...
    // FIXME: The SpAcc should be moved to the compute tree taco::ReductionNode
    // detection too.
    int spaccCount = 0;
    ...
    for (int count = 0; count < numIndexVars; count++) {
        ...
        IndexVar prevIndexVar =
            count == 0 ? nullptr
                       : getOrderedIndexVars().at(numIndexVars - count);

        auto crdDest =
            contains(resultHasSource, indexvar) && resultHasSource.at(indexvar)
                ? inputIterationCrdDst.at(indexvar)
                : SamIR();
        ...
        // FIXME: This will eventually need to be the iteration algebra for
        // fused kernels
        bool isIntersection =
            contains(contractionType, contractions.at(indexvar)) &&
            contractionType.at(contractions.at(indexvar));
        ...
        for (int ntp = 0; ntp < (int)getTensorPaths().size(); ntp++) {
            ...
            if (std::count(vars.begin(), vars.end(), indexvar) == 0 and
                ...
                if (count == 0) {
                    node = Repeat(inputValsArrays[tensorVar], indexvar,
                                  tensorVar, id, isRoot);
                } else {
        ...
        if (repeatNodes.size() > 1) {
            auto broadcast =
                Broadcast(repeatNodes, sam::SamEdgeType::repsig, id);
            id++;
        ...
        if (repeatSigGenNode.defined() && crdDest.defined()) {
            ...
            if (hasSparseAccumulation) {
                ...
                crdDest = Broadcast({crdDest, repeatSigGenNode},
                                    sam::SamEdgeType::crd, id, edgeName);
            } else {
                crdDest = Broadcast({crdDest, repeatSigGenNode},
                                    sam::SamEdgeType::crd, id);
            }
            id++;
        ...
        if (hasContraction) {
            ...
            bool printEdgeName =
                isCrdDrop || (!isCrdDrop && hasSparseAccumulation);
            auto edgeName = isCrdDrop               ? "in-" + indexvar.getName()
                            : hasSparseAccumulation ? indexvar.getName()
                                                    : "";
            if (isIntersection)
                contractNode =
                    taco::sam::Intersect(crdDest, contractOuts, indexvar, id,
                                         printEdgeName, edgeName);
            else
                contractNode = taco::sam::Union(crdDest, contractOuts, indexvar,
                                                id, printEdgeName, edgeName);
            id++;
        }
        ...
        for (int ntp = 0; ntp < (int)getTensorPaths().size(); ntp++) {
            ...
            if (std::count(vars.begin(), vars.end(), indexvar) > 0 and
                ...
                bool printEdgeName = hasContraction ||
                                     (!hasContraction && hasSparseAccumulation);
                if (count == 0) {
                    node = FiberLookup(
                        hasContraction ? contractNode
                                       : inputValsArrays[tensorVar],
                        hasContraction ? contractNode : crdDest, indexvar,
                        tensorVar, mode, id, edgeName, isRoot, true);
                } else {
                    auto prevSAMNode = hasContraction
                                           ? contractNode
                                           : nodeMap[prevIndexVar][ntp];
                    node = FiberLookup(
                        prevSAMNode, hasContraction ? contractNode : crdDest,
...
void SAMGraph::printInputIteration(std::ostream &os) {
    for (auto &tensorPath : getTensorPaths()) {
        ...
        os << "  "
           << " start input " << tensorName << " \t-> ";
        auto formats = getFormatMapping(tensorPath);
        for (auto indexvar : getOrderedIndexVars()) {
            auto vars = tensorPath.getVariables();
            if (std::count(vars.begin(), vars.end(), indexvar) > 0) {
                os << " " << tensorName << "_" << indexvar << " "
                   << formats[indexvar] << ": FiberLookup"
                   << " \t->";
            } else {
                os << " RepeatSigGen -> " << tensorName << "_" << indexvar
                   << " Repeat \t->";
            }
        }
        os << endl;
...
void SAMGraph::printAsDot(std::ostream &os) {
    ...
    if (resultVars.size() > 0) {
        os << "\n root -> " << resultVars[0] << " [style=dashed label=\""
           << resultName << "\"]";
    }
