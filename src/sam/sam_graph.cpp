...
SamIR SAMGraph::makeGraph() {
  ...
  // Generally it defaults to all index variables in expression, but this is not
  // true for expressions with inner ReductionNodes (sums)
  ...
  for (int count = 0; count < (int)getResultTensorPath().getSize(); count++) {
    ...
    if (count == 0) {
      ...
      crdSize =
          sizeMap.at(prevIndexVar).second + "*" + dimensionMap.at(indexvar);
    }
  ...
  SamIR resultWriteVals =
      FiberWrite(nullptr, getResultTensorPath().getAccess().getTensorVar(), -1,
                 "", valsSizeStr, id, true, true);
  id++;
  ...
  for (int count = 0; count < numIndexVars; count++) {
    ...
    if (std::count(resultVars.begin(), resultVars.end(), indexvar) > 0) {
      ...
      auto node =
          FiberWrite(indexvar, getResultTensorPath().getAccess().getTensorVar(),
  ...
  if (!reduction.empty()) {
    ...
    //            for (auto it =  reduction.rbegin(); it != reduction.rend();
    //            it++) {
    //                auto red = *it;
    //                switch (red) {
        ...
        function<void(const taco::ReductionNode *, Matcher *)>(
            [&](const taco::ReductionNode *op, Matcher *ctx) {
              ...
              if (op->var == getOrderedIndexVars().at(numIndexVars - 1) ||
                ...
                // accumulator. For example, a sparse accumulator for a vector
  ...
  for (int count = 0; count < (int)getOrderedIndexVars().size(); count++) {
    ...
    for (int outerCount = 0; outerCount < count; outerCount++) {
      ...
      bool isOuterResult =
          std::count(resultVars.begin(), resultVars.end(), outerIndexVar) > 0;
      hasOuterResult = hasOuterResult || isOuterResult;
    }
    hasOuterResult = hasOuterResult && (count > 0);

    bool innerIntersection = false;
    for (int innerCount = count; innerCount < (int)getOrderedIndexVars().size();
         innerCount++) {
    ...
    if (innerIntersection and hasOuterResult) {
      auto node = CrdDrop(inputIterationCrdDst[prevContractionVar],
                          inputIterationCrdDst[indexVar], prevContractionVar,
                          indexVar, id);
      id++;
  ...
  for (int count = 0; count < numIndexVars; count++) {
    ...
    IndexVar prevIndexVar =
        count == 0 ? nullptr : getOrderedIndexVars().at(numIndexVars - count);
    ...
    bool hasSparseAccumulation = isa<SparseAccumulator>(crdDest) ||
                                 isa<CrdHold>(crdDest) || isa<CrdDrop>(crdDest);

    // FIXME: This will eventually need to be the iteration algebra for fused
    // kernels
    bool isIntersection =
    ...
    if (repeatSigGenNode.defined() && crdDest.defined()) {
      ...
      if (hasSparseAccumulation) {
        ...
        crdDest = Broadcast({crdDest, repeatSigGenNode}, sam::SamEdgeType::crd,
                            id, edgeName);
      } else {
        crdDest =
            Broadcast({crdDest, repeatSigGenNode}, sam::SamEdgeType::crd, id);
      }
      id++;
    ...
    for (int ntp = 0; ntp < (int)getTensorPaths().size(); ntp++) {
      ...
      if (std::count(vars.begin(), vars.end(), indexvar) > 0 and
        ...
        bool printEdgeName =
            hasContraction || (!hasContraction && hasSparseAccumulation);
        if (count == 0) {
          node = FiberLookup(hasContraction ? contractNode
                                            : inputValsArrays[tensorVar],
                             hasContraction ? contractNode : crdDest, indexvar,
                             tensorVar, mode, id, edgeName, isRoot, true);
        } else {
          auto prevSAMNode =
              hasContraction ? contractNode : nodeMap[prevIndexVar][ntp];
          node = FiberLookup(prevSAMNode,
                             hasContraction ? contractNode : crdDest, indexvar,
                             tensorVar, mode, id, edgeName, isRoot, true);
        }
...
void SAMGraph::printInputIteration(std::ostream &os) {
  for (auto &tensorPath : getTensorPaths()) {
    ...
    for (auto indexvar : getOrderedIndexVars()) {
      auto vars = tensorPath.getVariables();
      if (std::count(vars.begin(), vars.end(), indexvar) > 0) {
        os << " " << tensorName << "_" << indexvar << " " << formats[indexvar]
           << ": FiberLookup"
           << " \t->";
      } else {
