...
void SAMPrinter::print(const SamIR &sam) { sam.accept(this); }
...
string SAMDotNodePrinter::printTensorFormats(const RootNode *op) {
    ...
    for (int i = 0; i < (int)op->tensors.size(); i++) {
        ...
        ss << t.getName() << "="
           << (formats.empty() ? "none" : util::join(formats, ""))
...
void SAMDotNodePrinter::visit(const FiberLookupNode *op) {
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        ...
        // comment << "\"type=fiberlookup,index=" << op->i.getName() <<
...
void SAMDotNodePrinter::visit(const FiberWriteNode *op) {
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        ...
        if (op->vals) {
            ...
            // comment << "\"type=fiberwrite,index=" << op->i.getName() <<
            // ",tensor=" << op->tensorVar.getName()
            //         << ",mode=" << std::to_string(op->mode)
            //         << ",format=" << op->modeFormat.getName();
...
void SAMDotNodePrinter::visit(const RepeatNode *op) {
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        ...
        std::stringstream comment;
        // comment << "\"type=repeat,index=" << op->i.getName() << ",tensor=" <<
        // op->tensorVar.getName() << root << "\"";
...
void SAMDotNodePrinter::visit(const JoinerNode *op) {
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        std::stringstream comment;
        // comment << "\"type=" << op->getNodeName() << ",index=" <<
        // op->i.getName() << "\"";
...
void SAMDotNodePrinter::visit(const SparseAccumulatorNode *op) {
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        ...
        for (auto ivar : op->ivarMap) {
            // comment << ",in" << to_string(ivar.first) << "=" <<
            // ivar.second.getName();
            if (to_string(ivar.first) == "0") {
                new_op->mutable_spacc()->set_inner_crd(ivar.second.getName());
            } else {
                new_op->mutable_spacc()->add_outer_crds(ivar.second.getName());
            }
        }
        // comment << "\"";
...
void SAMDotNodePrinter::visit(const CrdDropNode *op) {
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        std::stringstream comment;
        // comment << "\"type=crddrop,outer=" << op->outer << ",inner=" <<
        // op->inner << "\"";
...
void SAMDotNodePrinter::visit(const CrdHoldNode *op) {
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        std::stringstream comment;
        // comment << "\"type=crdhold,outer=" << op->outer << ",inner=" <<
        // op->inner << "\"";
        ...
        // if (prettyPrint) {
        //     os << " label=\"" << op->getName() << "\nouter="<< op->outer <<
        //     ",inner=" << op->inner << "\""; os << " color=orange shape=box
        //     style=filled";
        // }
        // if (printAttributes) {
        //     os <<   " type=\"" << "crdhold" << "\"" <<
...
void SAMDotEdgePrinter::print(const SamIR &sam) {
    ...
    std::sort(pg.mutable_operators()->begin(), pg.mutable_operators()->end(),
              [](const Operation &a, Operation &b) { return a.id() > b.id(); });
    ...
    // google::protobuf::io::FileOutputStream* output = new
    // google::protobuf::io::FileOutputStream(os);
    // // google::protobuf::io::OstreamOutputStream* output = new
    // google::protobuf::io::OstreamOutputStream(os); if
    // (!google::protobuf::TextFormat::Print(pg, output)) {
    ...
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        ...
        for (SamIR node : op->outputs) {
            if (node.defined()) {
                ...
                comment =
                    contains(op->edgeName, node) ? op->edgeName.at(node) : "";
                switch (op->type) {
...
void SAMDotEdgePrinter::visit(const FiberLookupNode *op) {
    if (!op->root) {
        ...
        if (curr_op) {
            id_to_op[op->nodeID]
                ...
                ->mutable_input_ref()
                ->mutable_id()
                ->set_id(chan_track.get_create_channel("out_ref",
                                                       curr_op->id() - 1));
        }
    }
    ...
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        ...
        if (op->out_crd.defined()) {
            printComment = op->printEdgeName;
            comment = contains(op->edgeName, op->out_crd)
                          ? op->edgeName.at(op->out_crd)
                          : "";
            edgeType = "crd";
            // os << tab << op->nodeID << " -> ";
            curr_op = id_to_op[op->nodeID];
            out_stream.cstream =
                curr_op->mutable_fiber_lookup()->mutable_output_crd();
            out_stream.cstream->set_name(comment);
            // chan_map[std::make_pair("out_crd", op->nodeID + 1)];
            curr_op->mutable_fiber_lookup()
                ->mutable_output_crd()
                ->mutable_id()
                ->set_id(chan_track.get_create_channel("out_crd", op->nodeID));
        ...
        if (op->out_ref.defined()) {
            printComment = op->printEdgeName;
            comment = contains(op->edgeName, op->out_ref)
                          ? op->edgeName.at(op->out_ref)
                          : "";
            edgeType = "ref";
            // os << tab << op->nodeID << " -> ";
            curr_op = id_to_op[op->nodeID];
            curr_op->mutable_fiber_lookup()
                ->mutable_output_ref()
                ->mutable_id()
                ->set_id(chan_track.get_create_channel("out_ref", op->nodeID));
            op->out_ref.accept(this);
        }
    ...
    if (!op->vals) {
        ...
        if (curr_op) {
            id_to_op[op->nodeID]
                ...
                ->mutable_input_crd()
                ->mutable_id()
                ->set_id(chan_track.get_create_channel("out_crd",
                                                       curr_op->id() - 1));
        }
    } else {
        id_to_op[op->nodeID]
        ...
        if (curr_op) {
            id_to_op[op->nodeID]
                ...
                ->mutable_input_val()
                ->mutable_id()
                ->set_id(chan_track.get_create_channel("out_val",
                                                       curr_op->id() - 1));
        }
    }
    ...
    if (curr_op->name() == "repsiggen") {
        ...
        if (curr_op) {
            id_to_op[op->nodeID]
                ...
                ->mutable_input_rep_sig()
                ->mutable_id()
                ->set_id(chan_track.get_create_channel("out_repsig",
                                                       curr_op->id() - 1));
        }
    } else {
        id_to_op[op->nodeID]->mutable_repeat()->mutable_input_ref()->set_name(
            label);
        if (curr_op) {
            id_to_op[op->nodeID]
                ->mutable_repeat()
...
void SAMDotEdgePrinter::visit(const RepeatSigGenNode *op) {
    ...
    id_to_op[op->nodeID]->mutable_repeatsig()->mutable_input_crd()->set_name(
        label);
    if (curr_op) {
        id_to_op[op->nodeID]
            ->mutable_repeatsig()
            ...
            ->mutable_id()
            ->set_id(
                chan_track.get_create_channel("out_crd", curr_op->id() - 1));
    }
    ...
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        if (op->out_repsig.defined()) {
            ...
            // os << tab << op->nodeID << " -> ";
            curr_op = id_to_op[op->nodeID];
            // out_stream.rsigstream =
            // curr_op->mutable_repeatsig()->mutable_output_rep_sig();
            // out_stream.rsigstream->set_name(edgeType);
            out_stream.type = edgeType;
            // chan_track.get_create_channel("out_repsig", op->nodeID);
            curr_op->mutable_repeatsig()
                ->mutable_output_rep_sig()
                ->mutable_id()
                ->set_id(
                    chan_track.get_create_channel("out_repsig", op->nodeID));
            op->out_repsig.accept(this);
...
void SAMDotEdgePrinter::visit(const JoinerNode *op) {
    ...
    if (edgeType == "ref") {
        int num_pairs =
            id_to_op[op->nodeID]->mutable_joiner()->input_pairs_size();
        Joiner::JoinBundle *bundle;
        if (num_pairs == 0 ||
            (full_joiner == 2 && id_to_op[op->nodeID]
                                     ->mutable_joiner()
                                     ->mutable_input_pairs(num_pairs - 1)
                                     ->has_ref())) {
        ...
        bundle = id_to_op[op->nodeID]->mutable_joiner()->mutable_input_pairs(
            num_pairs - 1);
        if (curr_op) {
            bundle->mutable_ref()->mutable_id()->set_id(
                chan_track.get_create_channel("out_ref", curr_op->id() - 1));
        ...
        int num_pairs =
            id_to_op[op->nodeID]->mutable_joiner()->input_pairs_size();
        Joiner::JoinBundle *bundle;
        if (num_pairs == 0 || id_to_op[op->nodeID]
                                  ->mutable_joiner()
                                  ->mutable_input_pairs(num_pairs - 1)
                                  ->has_crd()) {
            bundle = id_to_op[op->nodeID]->mutable_joiner()->add_input_pairs();
        ...
        bundle = id_to_op[op->nodeID]->mutable_joiner()->mutable_input_pairs(
            num_pairs - 1);
        if (curr_op) {
            bundle->mutable_crd()->mutable_id()->set_id(
                chan_track.get_create_channel("out_crd", curr_op->id() - 1));
    ...
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        if (op->out_crd.defined()) {
            ...
            curr_op = id_to_op[op->nodeID];
            out_stream.cstream =
                curr_op->mutable_joiner()->mutable_output_crd();
            out_stream.cstream->set_name(comment);
            // chan_track.get_create_channel("out_crd", op->nodeID);
            curr_op->mutable_joiner()
                ->mutable_output_crd()
                ->mutable_id()
                ->set_id(chan_track.get_create_channel("out_crd", op->nodeID));
            out_stream.type = edgeType;
        ...
        for (int i = 0; i < (int)op->out_refs.size(); i++) {
            ...
            if (out_ref.defined()) {
                ...
                curr_op = id_to_op[op->nodeID];
                // cout <<
                // string(&(curr_op->mutable_joiner()->mutable_input_pairs(0)->mutable_crd()->name().back()))
                // << endl; cout << string(&comment.back()) << endl; cout <<
                // endl;
                if (string(&comment.back()) ==
                    string(&(curr_op->mutable_joiner()
                                 ->mutable_input_pairs(0)
                                 ->mutable_crd()
                                 ->name()
                                 .back()))) {
                    out_stream.rstream =
                        curr_op->mutable_joiner()->mutable_output_ref1();
                    out_stream.rstream->set_name(comment);
                    curr_op->mutable_joiner()
                        ->mutable_output_ref1()
                        ->mutable_id()
                        ->set_id(
                            chan_track.create_channel("out_ref", op->nodeID));
                } else {
                    out_stream.rstream =
                        curr_op->mutable_joiner()->mutable_output_ref2();
                    out_stream.rstream->set_name(comment);
                    curr_op->mutable_joiner()
                        ->mutable_output_ref2()
                        ->mutable_id()
...
void SAMDotEdgePrinter::visit(const ArrayNode *op) {
    if (!op->root) {
        ...
        id_to_op[op->nodeID]->mutable_array()->mutable_input_ref()->set_name(
            label);
        if (curr_op) {
            id_to_op[op->nodeID]
                ->mutable_array()
    ...
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        if (op->out_val.defined()) {
            ...
            out_stream.type = "val";
            curr_op->mutable_array()
                ->mutable_output_val()
                ->mutable_id()
                ->set_id(chan_track.get_create_channel("out_val", op->nodeID));
            op->out_val.accept(this);
        }
...
void SAMDotEdgePrinter::visit(const ComputeNode *op) {
    ...
    if (op->getNodeName() != "reduce") {
        ValStream *alu_val =
            id_to_op[op->nodeID]->mutable_alu()->mutable_vals()->add_inputs();
        alu_val->set_name(label);
        if (curr_op) {
            int num_inputs = id_to_op[op->nodeID]
                                 ->mutable_alu()
                                 ->mutable_vals()
                                 ->inputs_size();
            id_to_op[op->nodeID]
                ->mutable_alu()
                ->mutable_vals()
                ->mutable_inputs(num_inputs - 1)
                ->mutable_id()
                ->set_id(chan_track.get_create_channel("out_val",
                                                       curr_op->id() - 1));
        }
    } else {
        id_to_op[op->nodeID]->mutable_reduce()->mutable_input_val()->set_name(
            label);
        if (curr_op) {
            id_to_op[op->nodeID]
                ->mutable_reduce()
    ...
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        if (op->out_val.defined()) {
            ...
            if (op->getNodeName() != "reduce") {
                out_stream.vstream =
                    curr_op->mutable_alu()->mutable_vals()->mutable_output();
                curr_op->mutable_alu()
                    ->mutable_vals()
                    ->mutable_output()
                    ->mutable_id()
                ...
                out_stream.vstream =
                    curr_op->mutable_reduce()->mutable_output_val();
                out_stream.vstream->set_name("reduce_val");
                curr_op->mutable_reduce()
                    ->mutable_output_val()
                    ->mutable_id()
                    ->set_id(
                        chan_track.get_create_channel("out_val", op->nodeID));
...
void SAMDotEdgePrinter::visit(const SparseAccumulatorNode *op) {
    ...
    if (edgeType == "crd") {
        if (string(&label.back()) ==
            id_to_op[op->nodeID]->mutable_spacc()->inner_crd()) {
            id_to_op[op->nodeID]
                ->mutable_spacc()
                ->mutable_input_inner_crd()
                ->set_name(label);
            if (curr_op) {
                id_to_op[op->nodeID]
                    ->mutable_spacc()
            ...
            id_to_op[op->nodeID]
                ->mutable_spacc()
                ->add_input_outer_crds()
                ->set_name(label);
            const int num_inputs =
                id_to_op[op->nodeID]->mutable_spacc()->input_outer_crds_size();
            if (curr_op) {
                id_to_op[op->nodeID]
                    ->mutable_spacc()
                    ->mutable_input_outer_crds(num_inputs - 1)
                    ->mutable_id()
                    ->set_id(chan_track.get_create_channel("out_crd",
                                                           curr_op->id() - 1));
            }
        ...
        // std::cout << label << endl;
        id_to_op[op->nodeID]->mutable_spacc()->mutable_input_val()->set_name(
            label);
        if (curr_op) {
            id_to_op[op->nodeID]
                ->mutable_spacc()
    ...
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        ...
        for (auto out_crd : op->out_crds) {
            ...
            if (string(&comment.back()) ==
                id_to_op[op->nodeID]->mutable_spacc()->inner_crd()) {
                std::cout << "inner: " << comment << endl;
                // os << tab << op->nodeID << " -> ";
                curr_op = id_to_op[op->nodeID];
                curr_op->mutable_spacc()->mutable_output_inner_crd()->set_name(
                    comment);
                // chan_track.get_create_channel("out_crd", op->nodeID);
                curr_op->mutable_spacc()
                    ->mutable_output_inner_crd()
                    ->mutable_id()
                    ->set_id(
                        chan_track.get_create_channel("out_crd", op->nodeID));
                out_stream.cstream =
...
void SAMDotEdgePrinter::visit(const CrdDropNode *op) {
    ...
    if (string(&label.back()) ==
        id_to_op[op->nodeID]->mutable_coord_drop()->inner_crd()) {
        id_to_op[op->nodeID]
            ->mutable_coord_drop()
            ->mutable_input_inner_crd()
            ->set_name(label);
    ...
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        if (op->out_outer_crd.defined()) {
            ...
            // os << tab << op->nodeID << " -> ";
            curr_op = id_to_op[op->nodeID];
            curr_op->mutable_coord_drop()->mutable_output_outer_crd()->set_name(
                comment);
            out_stream.cstream =
                curr_op->mutable_coord_drop()->mutable_output_outer_crd();
            out_stream.type = "crd";
            curr_op->mutable_coord_drop()
                ->mutable_output_outer_crd()
                ->mutable_id()
                ->set_id(chan_track.get_create_channel("out_crd", op->nodeID));
            op->out_outer_crd.accept(this);
        }
        ...
        if (op->out_inner_crd.defined()) {
            ...
            // os << tab << op->nodeID << " -> ";
            curr_op = id_to_op[op->nodeID];
            curr_op->mutable_coord_drop()->mutable_output_inner_crd()->set_name(
                comment);
            out_stream.cstream =
                curr_op->mutable_coord_drop()->mutable_output_inner_crd();
            out_stream.type = "crd";
            curr_op->mutable_coord_drop()
                ->mutable_output_inner_crd()
                ->mutable_id()
                ->set_id(chan_track.create_channel("out_crd", op->nodeID));
            // chan_track.get_create_channel("out_crd", op->nodeID);
            op->out_inner_crd.accept(this);
        }
...
void SAMDotEdgePrinter::visit(const CrdHoldNode *op) {
    ...
    if (string(&label.back()) ==
        id_to_op[op->nodeID]->mutable_coord_hold()->inner_crd()) {
        id_to_op[op->nodeID]
            ->mutable_coord_hold()
            ->mutable_input_inner_crd()
            ->set_name(label);
    ...
    if (std::count(printedNodes.begin(), printedNodes.end(), op->nodeID) == 0) {
        if (op->out_outer_crd.defined()) {
            ...
            // os << tab << op->nodeID << " -> ";
            curr_op = id_to_op[op->nodeID];
            curr_op->mutable_coord_hold()->mutable_output_outer_crd()->set_name(
                comment);
            out_stream.cstream =
                curr_op->mutable_coord_hold()->mutable_output_outer_crd();
            out_stream.type = "crd";
            // chan_track.get_create_channel("out_crd", op->nodeID);
            curr_op->mutable_coord_hold()
                ->mutable_output_outer_crd()
                ->mutable_id()
                ->set_id(chan_track.get_create_channel("out_crd", op->nodeID));
            op->out_outer_crd.accept(this);
        ...
        if (op->out_inner_crd.defined()) {
            ...
            // os << tab << op->nodeID << " -> ";
            curr_op = id_to_op[op->nodeID];
            curr_op->mutable_coord_hold()->mutable_output_inner_crd()->set_name(
                comment);
            out_stream.cstream =
                curr_op->mutable_coord_hold()->mutable_output_inner_crd();
            // chan_track.get_create_channel("out_crd", op->nodeID);
            curr_op->mutable_coord_hold()
                ->mutable_output_inner_crd()
                ->mutable_id()
